...
def makeBothParams(kilosortDir:Path):
    paramfile = kilosortDir/'params.py'
    namespace = {}
    ...
    dat_path = Path(namespace["dat_path"][0])
    ...
    bcParams  = get_default_parameters(str(kilosortDir), 
                                  raw_file=str(realbinPath),
                                  meta_file=None,
                                  kilosort_version=4)
    bcParams['gain_to_uV'] = gain_to_uV
    bcParams['ephys_sample_rate'] = sample_rate
    ...
    bcParams['savePlots'] = False
    bcParams["plotDetails"] = False
    bcParams['plotGlobal'] = True
    bcParams['saveAsTSV'] = False
    bcParams['presenceRatioBinSize'] = None
    bcParams["tauR_values"]  = np.array([0.001]) #1 ms refactory period think this is fine for birds 
    bcParams['tauR_valuesMax'] = 0.001
    bcParams['tauR_valuesMin'] = 0.001
    if 'rpv_method' in bcParams.keys():
        bcParams.pop('rpv_method')
    ...
    ##now ephysParam
    ephysParams = get_ephys_parameters(str(kilosortDir), 
                                  raw_file=str(realbinPath),
                                  meta_file=None)
    ephysParams['gain_to_uV'] = gain_to_uV
    ephysParams['ephys_sample_rate'] = sample_rate
    ephysParams['verbose'] = False
    ephysParams['nChannels'] = n_channels_dat 
    ephysParams['nSyncChannels'] = 0
    ephysParams[ 'nRawSpikesToExtract'] = nRawSpikesToExtract
    ephysParams['saveMultipleRaw']= False
    ephysParams['spikeWidth'] = 61
    ephysParams['reextractRaw']= False
    ephysParams['saveSpikes_withoutDuplicates'] = False

    return bcParams,ephysParams
...
def get_all_quality_metrics(
    ...
    for unit_idx in tqdm(range(unique_templates.size), bar_format=bar_description,disable=True):
...
def makeChunkIdxes(raw_data_file,ephys_sample_rate,nChannels,chunkSizeSec):

    fileSizeBytes = Path(raw_data_file).stat().st_size
    nFileSamp = fileSizeBytes//(2*nChannels)
    chunkSamp = int(ephys_sample_rate * chunkSizeSec)
...
def run_bombcellCHUNKEDALL(ks_dir, save_pathRun, param, ephysParam, chunkSizeSec = 600,save_figures=False, return_figures=False):
    ...
    save_pathRun : string
        The path to the directory to save the bombcell results. each chunk will now live in this savepath
    param : dict
        The param dictionary
    ...
    """
    #start by making my chunks
    chunkStarts,chunkStops = makeChunkIdxes(raw_data_file= param['raw_data_file'],nChannels=param['nChannels'],ephys_sample_rate = param['ephys_sample_rate'],chunkSizeSec=chunkSizeSec)
    save_pathRun.mkdir(exist_ok = True,parents=True)
    np.save(save_pathRun/'chunkIDXs.npy',np.c_[chunkStarts,chunkStops])
    nChunks = len(chunkStarts)
    save_pathRun = Path(save_pathRun)
    if param.get("verbose", False):
        print("🚀 Starting BombCell CHUNKED quality metrics pipeline...")
        print(f"📁 Processing data from: {ks_dir}")
        print(f"Results will be saved to: {save_pathRun}")
        
        print("\nLoading ephys data...")
    (
            spike_times_samplesFULL,
            spike_clustersFULL, # actually spike_templates, but they're the same in bombcell
            template_waveforms,
            template_amplitudesFULL,
            pc_featuresFULL,
            pc_features_idx,
            channel_positions,
        ) = load_ephys_data(ks_dir)
    # pre-load peak channels from templates before extracting raw waveforms- moved outside loop
    maxChannels = qm.get_waveform_peak_channel(template_waveforms)
    print(f"Chunk Time Sec is {chunkSizeSec}, with n  = {nChunks} chunks")
    ##start processesing loop
    for chunkN,(start,end) in enumerate(tqdm(zip(chunkStarts,chunkStops),total=nChunks,desc='Processesing Chunks')):
        
        chunkSpikeIdx0 = np.searchsorted(spike_times_samplesFULL,start,side = 'left') ###keep valid spiketimes that wont be truncated at file edges 
        chunkSpikeIdx1 = np.searchsorted(spike_times_samplesFULL,end,side = 'right')

        #subset to indiced within this chunk
        spike_times_samples = spike_times_samplesFULL[chunkSpikeIdx0:chunkSpikeIdx1]
        spike_clusters  = spike_clustersFULL[chunkSpikeIdx0:chunkSpikeIdx1]
        template_amplitudes  = template_amplitudesFULL[chunkSpikeIdx0:chunkSpikeIdx1]
        pc_features = pc_featuresFULL[chunkSpikeIdx0:chunkSpikeIdx1,:,:]

        save_path = save_pathRun/f"chunk_{chunkN}"
